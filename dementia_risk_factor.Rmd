...
graphviz.chart(bn.bayes_imputed_custom, grid = TRUE, main = "Original BN")
#predictor error
...
sum_a<-0
for (i in 1:10){
aaa<-predict(bn.bayes_imputed_custom,"AD",eassy_share_important_variables_large60_wave_5_estonia)

l1 <-nrow(eassy_share_important_variables_large60_wave_5_estonia)
prob_a <- length(which(aaa!=eassy_share_important_variables_large60_wave_5_estonia$AD))/l1
sum_a <- sum_a+prob_a
}
cat("\n bn.bayes_imputed_custom error rate(drop na data set,wave 5):",sum_a/10)
...
eassy_share_important_variables_large60_wave_4_estonia_narm_AD_cognitive_0_1 <-
  ...
  for (i in 1:n){
   ...
   # DS <- ifelse(DS<="[0,2)",1,ifelse(DS<="[2,5)",2,ifelse(DS<="[5,8)",3,4)))
    ...
    eassy_share_important_variables_large60_wave_4_estonia_narm_AD_cognitive_0_1$pred[i]<-bn.bayes_imputed_custom$AD$prob[1,Age,VA,EDUL]
  }
